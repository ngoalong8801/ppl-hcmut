...
class StaticChecker(BaseVisitor):
    ...
    def visitAssign(self, ast, c):
        ...
        if(type(typeLhs) is not type(typeExp)):
            if(not ((type(typeLhs) is FloatType) and (type(typeExp) is IntType))):
    ...
    def visitBinaryOp(self, ast, c):
        ...
        if ast.op in ['&&', '||']:
            ...
            if not(isinstance(typeLeft, BoolType) and isinstance(typeRight, BoolType)):
                raise TypeMismatchInExpression(ast)
            return BoolType()
        elif ast.op == '==.':
