...
TARGET = '../target/main/d95/parser' if os.name == 'posix' else os.path.normpath('../target/')
locpath = ['./main/d95/parser/','./main/d95/astgen/','./main/d95/utils/','./main/d95/checker']
for p in locpath:
...
def main(argv):
    if len(argv) < 1:
        ...
        subprocess.run(["java","-jar",ANTLR_JAR,"-o","../target","-no-listener","-visitor","main/d95/parser/D95.g4"])
    elif argv[0] == 'clean':
