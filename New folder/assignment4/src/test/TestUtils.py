...
if not './main/d95/parser/' in sys.path:
    sys.path.append('./main/d95/parser/')
if os.path.isdir('../target/main/d95/parser') and not '../target/main/d95/parser/' in sys.path:
    sys.path.append('../target/main/d95/parser/')

from D95Lexer import D95Lexer
...
class TestAST:
    ...
    def test(inputdir,outputdir,num):
        ...
        lexer = D95Lexer(FileStream(inputdir + "/" + str(num) + ".txt"))
        tokens = CommonTokenStream(lexer)
        parser = D95Parser(tokens)
        tree = parser.program()
        asttree = ASTGeneration().visit(tree)
